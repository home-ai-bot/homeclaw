...
func (al *AgentLoop) InjectToolWSHandler(cfg *config.Config) {
	...
	// Type assert channelManager to *channels.Manager
	cm, ok := al.channelManager.(*channels.Manager)
	if !ok {
		logger.Warnf("[InjectToolWSHandler] channelManager is not *channels.Manager, skipping")
		return
	}

	al.homeclaw.InjectToolWSHandler(cm, defaultAgent.Tools, cfg)
}
