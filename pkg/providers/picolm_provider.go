...
func NewPicoLMProvider(cfg config.PicoLMProviderConfig) (*PicoLMProvider, error) {
	...
	if binary != "" {
		...
		// On Windows, skip executable bit check as Windows doesn't use Unix permission bits
		// Instead, check if the file has .exe, .bat, or .cmd extension
		if runtime.GOOS != "windows" {
			if info.Mode()&0111 == 0 {
				return nil, fmt.Errorf("picolm: binary %q is not executable", binary)
			}
		} else {
			// On Windows, verify it's an executable file by extension
			ext := strings.ToLower(filepath.Ext(binary))
			if ext != ".exe" && ext != ".bat" && ext != ".cmd" && ext != ".com" {
				return nil, fmt.Errorf("picolm: binary %q does not have a valid executable extension (.exe, .bat, .cmd, .com)", binary)
			}
		}
	}
...
func (p *PicoLMProvider) Chat(ctx context.Context, messages []Message, tools []ToolDefinition, model string, options map[string]interface{}) (*LLMResponse, error) {
	...
	logger.DebugCF("provider.picolm", "executing picolm", map[string]any{
		"binary":      p.binary,
		"model":       p.model,
	...
	if err != nil {
		...
		logger.ErrorCF("provider.picolm", "picolm process failed", map[string]any{
			"binary":     p.binary,
			"model":      p.model,
			"args":       args,
	...
	if output == "" {
		logger.DebugCF("provider.picolm", "picolm returned empty output", map[string]any{
			"model": p.model,
		})
		return &LLMResponse{
			Content:      "",
	...
	logger.DebugCF("provider.picolm", "picolm response received", map[string]any{
		"model":         p.model,
